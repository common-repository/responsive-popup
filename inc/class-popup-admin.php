...
//			);
	...
	static public function post_columns( $post_columns ) {
		...
		$new_columns['po_name'] = __( 'Name', PO_LANG );
		$new_columns['po_cond'] = __( 'Visibility', PO_LANG );
	...
	static public function post_column_content( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'po_cond':
				...
				<?php endforeach; ?>
				<?php if ( ! $rule_count ) : ?>
					<span><?php _e( 'Always Show PopUp', PO_LANG ); ?></span>
				<?php endif; ?>
				</div>
				<?php
				break;
